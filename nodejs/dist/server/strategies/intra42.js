...
passport.use('intra42', new OAuth2Strategy({
    ...
    try {
        ...
        // chercher le user
        const [result] = await pool.query('SELECT id, mail, password, username, language, two_fa FROM `User` WHERE mail = ?', [data.email]);
        // le user existe pas
        if (result.length === 0) {
            try {
                const [user_result] = await pool.query('INSERT INTO `User` (mail, password, username, language) VALUES (?, ?, ?, ?)', [data.email, '', data.login, 'fr']);
                var file = null;
                if (image_url)
                    file = await downloadImage(image_url, user_result.insertId);
                const [profile_result] = await pool.query('INSERT INTO `Profile` (id_user, elo, xp, path_img) VALUES (?, ?, ?, ?)', [user_result.insertId, 400, 0, file]);
            }
            catch (err) {
                await pool.query('DELETE FROM `User` WHERE mail=?', [data.email]);
                console.error(err);
                done(null, user);
                return;
            }
        ...
        // le user existe
        const [finalUserRes] = await pool.query('SELECT id, mail, password, username, language, two_fa FROM `User` WHERE mail = ?', [data.email]);
        const finalUser = finalUserRes[0];
        const [profile] = await pool.query('SELECT path_img FROM `Profile` WHERE id_user = ?', [finalUser.id]);
