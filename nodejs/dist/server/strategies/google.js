...
const googleCallBack = async (accessToken, refreshToken, profile, done) => {
    ...
    // chercher le user
    try {
        const [result] = await pool.query('SELECT id, mail, password, username, language, two_fa FROM `User` WHERE mail = ?', [email]);
        // le user existe pas
        if (result.length === 0) {
            try {
                const [user_result] = await pool.query('INSERT INTO `User` (mail, password, username, language) VALUES (?, ?, ?, ?)', [email, '', profile._json.name, 'fr']);
                var file = null;
                if (image_url)
                ...
                await pool.query('DELETE FROM `User` WHERE mail=?', [email]);
                console.error(err);
            }
        }
    }
    ...
    // le user existe
    try {
        const [finalUserRes] = await pool.query('SELECT id, mail, password, username, language, two_fa FROM `User` WHERE mail = ?', [email]);
