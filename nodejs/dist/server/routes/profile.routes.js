...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [username] = await pool.query('SELECT username AS username FROM `User` WHERE id = ?', [userId]);
        const [profileRows] = await pool.query('SELECT * FROM `Profile` WHERE id_user = ?', [userId]);
        ...
        const [statsRows] = await pool.query(`SELECT 
			...
			FROM Game
			WHERE id_player_one = ? OR id_player_second = ?`, [userId, userId, userId, userId]);
        const [friends] = await pool.query('SELECT COUNT(*) AS nb_friends FROM friends WHERE id_user_1 = ?', [userId]);
        ...
        const [user_achievements] = await pool.query('SELECT * FROM User_achievements WHERE id_user = ?', [userId]);
        var finalTab = [];
        var row;
        for (var i = 0; achievements[i]; ++i) {
            finalTab[i] = {};
            row = user_achievements.filter((a) => a.id_achievement === achievements[i].id);
