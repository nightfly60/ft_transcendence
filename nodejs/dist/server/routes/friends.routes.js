...
router.get('/status/:targetId', requireAuth, async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(`SELECT * FROM friends WHERE (id_user_1 = ? AND id_user_2 = ?)`, [userId, targetId]);
...
router.post('/add/:targetId', requireAuth, async (req, res) => {
    ...
    try {
        const [existing] = await pool.query(`SELECT * FROM friends WHERE (id_user_1 = ? AND id_user_2 = ?)`, [userId, targetId]);
        if (existing.length > 0)
            return res.status(400).json({ error: 'Deja amis' });
        await pool.query(`INSERT INTO friends (id_user_1, id_user_2) VALUES (?, ?)`, [userId, targetId]);
...
router.delete('/remove/:targetId', requireAuth, async (req, res) => {
    ...
    const targetId = parseInt(req.params['targetId']);
    try {
        await pool.query(`DELETE FROM friends WHERE (id_user_1 = ? AND id_user_2 = ?)`, [userId, targetId]);
...
router.get('/online/:targetId', requireAuth, async (req, res) => {
    const targetId = parseInt(req.params['targetId']);
    try {
        const [rows] = await pool.query(`SELECT last_seen FROM User WHERE id = ?`, [targetId]);
        if (!rows.length)
...
router.get('/list/:id', requireAuth, async (req, res) => {
    ...
    try {
        const [friends] = await pool.query(`SELECT 
			u.id,
			u.username,
			p.path_img,
			p.elo,
