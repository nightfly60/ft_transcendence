 ...
 */
function simulateMove(board, fromR, fromC, toR, toC, ctx) {
    ...
    if (piece.type === 'K' && Math.abs(toC - fromC) === 2) {
        if (toC === 6) {
            sim[fromR][5] = sim[fromR][7];
            sim[fromR][7] = null;
        }
        else {
            sim[fromR][3] = sim[fromR][0];
            sim[fromR][0] = null;
        }
    }
    return sim;
 ...
 */
export function getValidMoves(board, row, col, ctx = DEFAULT_CONTEXT) {
    const piece = board[row][col];
    if (!piece)
        return [];
    return getRawMoves(board, row, col, ctx).filter(([tr, tc]) => {
        if (isKingInCheck(simulateMove(board, row, col, tr, tc, ctx), piece.color))
            return false;
        if (piece.type === 'K' && Math.abs(tc - col) === 2) {
            if (isKingInCheck(board, piece.color))
                return false;
            if (isKingInCheck(simulateMove(board, row, col, row, (col + tc) / 2, ctx), piece.color))
                return false;
        }
        return true;
    });
 ...
 */
export function computeGameStatus(board, color, ctx = DEFAULT_CONTEXT) {
    const inCheck = isKingInCheck(board, color);
    const hasLegal = Array.from({ length: 8 }, (_, r) => Array.from({ length: 8 }, (_, c) => board[r][c]?.color === color && getValidMoves(board, r, c, ctx).length > 0)).flat().some(Boolean);
    if (!hasLegal && inCheck)
        return 'checkmate';
    if (!hasLegal)
        return 'stalemate';
