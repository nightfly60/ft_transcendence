 ...
 */
function updateCastlingRights(rights, fromR, fromC, toR, toC, piece) {
    const r = { ...rights };
    if (piece.type === 'K') {
        if (piece.color === 'w') {
            r.wK = false;
 ...
 */
export function applyMove(board, fromR, fromC, toR, toC, options = {}) {
 ...
 */
export function computeValidMovesMap(board, color, ctx = DEFAULT_CONTEXT) {
    const map = {};
    for (let r = 0; r < 8; r++)
        for (let c = 0; c < 8; c++)
