 ...
 */
export async function updateElo(winnerId, whiteUserId, blackUserId) {
    const [rows] = await pool.query('SELECT id_user, elo FROM Profile WHERE id_user IN (?, ?)', [whiteUserId, blackUserId]);
    const eloMap = {};
    rows.forEach(r => { eloMap[r['id_user']] = Number(r['elo']); });
