 ...
 */
export function registerDraw(io, socket) {
    socket.on('propose_draw', ({ gameId }) => {
        const game = multiGames.get(gameId);
        if (!game || game.gameStatus !== 'playing' && game.gameStatus !== 'check')
            return;
    ...
    socket.on('accept_draw', async ({ gameId }) => {
        const game = multiGames.get(gameId);
        if (!game || !drawOffers.has(gameId))
            return;
        drawOffers.delete(gameId);
        const drawn = { ...game, gameStatus: 'draw' };
