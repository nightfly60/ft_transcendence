...
async function fetchUsernames(userIds) {
    const [rows] = await pool.query('SELECT id, username FROM `User` WHERE id IN (?, ?)', userIds);
    const map = {};
 ...
 */
async function rejoinGame(socket, gameId, game, userId) {
    const color = userId === game.whiteUserId ? 'w' : 'b';
    if (color === 'w')
 ...
 */
async function createGame(io, socket, waitingSocketId) {
    ...
    const whiteUserId = whiteSocket.data.userId;
    const blackUserId = blackSocket.data.userId;
    //   console.log(`[find_game] match: white=${white}(userId=${whiteUserId}) vs black=${black}(userId=${blackUserId})`);
    let result;
    try {
        [result] = await pool.query('INSERT INTO Game (timestamp, id_player_one, id_player_second) VALUES (NOW(), ?, ?)', [whiteUserId, blackUserId]);
    }
    catch (err) {
    ...
    const game = { ...makeGame(), white, black, whiteUserId, blackUserId };
 ...
 */
export function registerFindGame(io, socket) {
