 ...
 */
export async function finalizeGame(gameId, whiteUserId, blackUserId, nbMoves, winnerId) {
    playerGames.delete(whiteUserId);
    playerGames.delete(blackUserId);
    await pool.query('UPDATE Game SET id_winner = ?, nb_cuts = ? WHERE id = ?', [winnerId ?? null, nbMoves, Number(gameId)]);
