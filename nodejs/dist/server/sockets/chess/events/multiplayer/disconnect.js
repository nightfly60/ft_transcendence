 ...
 */
function startForfeitTimer(io, socket, gameId, userId) {
    ...
    const timer = setTimeout(async () => {
        ...
        if (!currentGame || currentGame.gameStatus === 'checkmate' || currentGame.gameStatus === 'stalemate' || currentGame.gameStatus === 'draw' || currentGame.gameStatus === 'resign')
            return;
        const resignColor = userId === currentGame.whiteUserId ? 'w' : 'b';
        const forfeited = { ...currentGame, turn: resignColor, gameStatus: 'resign' };
 ...
 */
export function registerLeaveGame(io, socket) {
