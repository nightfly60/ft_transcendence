...
export function registerSoloIAEvents(io, socket) {
    socket.on('start_solo_ia', async (data = {}) => {
        ...
        try {
            const [result] = await pool.query('INSERT INTO Game (timestamp, id_player_one) VALUES (NOW(), ?)', [socket.data.userId]);
            const gameId = String(result.insertId);
            // console.log(`[soloIA] game créée gameId=${gameId}`);
            const game = { ...makeGame(), iaLevel: level, playerColor: 'w' };
    ...
    socket.on('solo_resign', ({ gameId }) => {
        ...
        if (!game || game.gameStatus !== 'playing')
            return;
        const resigned = { ...game, gameStatus: 'checkmate' };
