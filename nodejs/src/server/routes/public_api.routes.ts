...
router.get('/getKey', async (req, res) => {
	...
	try
	{
		const [userAPI]: any = await pool.query(
			`SELECT * FROM User_API WHERE id_user = ?`,
			[user.id]
		);
		if (userAPI.length === 0)
...
router.post('/genKey', async (req, res) => {
	...
	try
	{
		...
		await pool.query(
			"INSERT INTO User_API (id_user, usages, reset_date, secret_key) VALUES (?, 0, ?, ?)",
			[user.id, datetime, key]
		);
