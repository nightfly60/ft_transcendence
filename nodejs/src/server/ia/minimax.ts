...
export function evalPosition(chess: Chess): number
{
	let score = 0;
...
export function minimax(chess: Chess, depth: number, isMaximizing: boolean, alpha: number = -Infinity, beta: number = Infinity): number
{
	if (depth == 0)
