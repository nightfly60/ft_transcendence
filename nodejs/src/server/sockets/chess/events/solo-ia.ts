...
export function registerSoloIAEvents(io: Server, socket: Socket): void {
	...
	socket.on('solo_ia_move', async ({ gameId, from, to, promotion, }: { gameId: string; from: string; to: string; promotion?: string }) => {
		const game = soloIAGames.get(gameId); // on recup la partie
		if (!game || game.gameStatus === 'checkmate' || game.gameStatus === 'stalemate') return ; // elle est terminee ?
		...
		if (updated.gameStatus === 'checkmate' || updated.gameStatus === 'stalemate') return; // est ce que la partie est finie ?
		await playIAMove(gameId, updated, socket); // on joue le coup de l'IA
	});

	socket.on('solo_resign', ({ gameId }: { gameId: string }) => {
		const game = soloIAGames.get(gameId); // on recup la partie
		if (!game || game.gameStatus === 'checkmate' || game.gameStatus === 'stalemate') return ; // finie ??

		const resigned: SoloIAGame = {
			...game,
