...
import {
  PieceType, PieceColor, GameContext, ChessGame, DEFAULT_CONTEXT,
} from './types.js';
import {
 ...
 */
export function makeGame(): ChessGame {
  return {
    board: initBoard(),
    turn: 'w',
    gameStatus: 'playing',
 ...
 */
export function getCtx(game: ChessGame): GameContext {
 ...
 */
export function buildGameState(game: ChessGame) {
  return {
    board: game.board,
    turn: game.turn,
...
export function applyMoveToGame(
  ...
  const { board: newBoard, captured, newEnPassantTarget, newCastlingRights } =
    applyMove(game.board, fromR, fromC, toR, toC, { promotion, ctx });

  const nextTurn: PieceColor = game.turn === 'w' ? 'b' : 'w';
  const newCtx: GameContext  = { enPassantTarget: newEnPassantTarget, castlingRights: newCastlingRights };
...
): { fromR: number; fromC: number; toR: number; toC: number } | null {
 ...
 */
export function toPromotionPiece(raw?: string): PieceType {
  return (['Q', 'R', 'B', 'N'] as PieceType[]).includes(raw as PieceType)
    ? raw as PieceType
    : 'Q';
}
